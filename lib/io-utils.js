...
const removeLock = (lockFile, maxRetries, callback) => {
  ...
  unlink(lockFile)
      ...
      .catch((err) => {
        if (err.code === 'ENOENT') {
          console.error('Lock for file ' + lockFile + ' was already removed.');
          callback();
        } else if (maxRetries > 0) {
          console.error('Attempted to remove lock but unknown error occured. Trying again');
          setTimeout(() => removeLock(lockFile, --maxRetries, callback), 100 + Math.random() * 50);
        } else {
          console.error('Failed to remove lock for file ' + lockFile);
          console.error(err);
