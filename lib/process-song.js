...
const processSong = (config, root, collection, name, id, callback) => {
  ...
  const processor = getProcessor(config, song, newName, lockFile, zippedData, callback);
 ...
 * @returns {function()}
 */
const getProcessor = (config, song, newName, lockFile, zippedData, callback) => {
  ...
  const destDirectory = path.join(config.outputDir, 'processed', firstFolder, secondFolder);
  ...
  return () => {
    ioUtils.access(destFile)
        .then(() => {
          ...
          return {directory: path.join(config.duplicateDir, firstFolder), file: newName + '.zip', isDuplicate: true};
        })
        .catch(() => {
          // File does not exist
          return {directory: path.join(config.outputDir, 'processed', firstFolder, secondFolder), file: newName + '.zip'};
        })
        .then((dest) => {
